...
describe('Schema - ZeroOmega Export Compatibility', () => {
  ...
  beforeAll(() => {
    ...
    const exportPath = join(
      ...
      'ZeroOmegaExport_Company_Example.bak'
    );
    const raw = readFileSync(exportPath, 'utf-8');
    exportData = JSON.parse(raw) as OmegaExport;
  });
  ...
  describe('Company Profile (FixedProfile)', () => {
    ...
    it('should have fallbackProxy configuration', () => {
      const fixedProfile = companyProfile as FixedProfile;
    ...
    it('should have correct bypass conditions', () => {
      const fixedProfile = companyProfile as FixedProfile;
      const bypassPatterns = fixedProfile.bypassList!.map((c) => (c as BypassCondition).pattern);
  ...
  describe('Auto Switch Profile (SwitchProfile)', () => {
    ...
    it('should have defaultProfileName = direct', () => {
      const switchProfile = autoSwitchProfile as SwitchProfile;
    ...
    it('should map specific domains to Company profile', () => {
      ...
      const patterns = companyRules.map(
        (r) => (r.condition as HostWildcardCondition).pattern
      );
    ...
    it('should map Google domains to direct profile', () => {
      ...
      const patterns = directRules.map((r) => (r.condition as HostWildcardCondition).pattern);
  ...
  describe('Export Round-trip', () => {
    it('should serialize back to identical JSON structure', () => {
      ...
      const deserialized = JSON.parse(serialized) as OmegaExport;
    ...
    it('should preserve nested structures', () => {
      ...
      const originalCompany = exportData['+Company'] as FixedProfile;
      const deserializedCompany = deserialized['+Company'] as FixedProfile;
  ...
  describe('Schema Version 2 Compliance', () => {
    ...
    it('should support all required profile types', () => {
      ...
      const profileTypes = Object.keys(exportData)
        ...
        .map((k) => exportData[k as keyof OmegaExport])
        .map((p) => (p as Profile).profileType);
    ...
    it('should support all required condition types', () => {
      ...
      const company = exportData['+Company'] as FixedProfile;
      company.bypassList?.forEach((c) => allConditions.push(c.conditionType));

      // Collect from SwitchProfile rules
      const autoSwitch = exportData['+auto switch'] as SwitchProfile;
