...
export class RegexValidator {
   ...
   */
  static compileSafe(pattern: string, flags: string = 'i'): RegExp {
    try {
   ...
   */
  private static countAlternations(pattern: string): number {
    let count = 0;
   ...
   */
  private static countQuantifiers(pattern: string): number {
    let count = 0;
   ...
   */
  private static isEscaped(pattern: string, index: number): boolean {
