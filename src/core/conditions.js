...
export class ConditionMatcher {
     ...
     */
    static match(condition, context) {
        switch (condition.conditionType) {
            ...
            case 'HostLevelsCondition':
                return this.matchHostLevels(condition.minValue ?? 1, condition.maxValue ?? -1, context);
            ...
            default:
                return {
                    matched: false,
                    reason: `Unknown condition type: ${condition.conditionType}`,
     ...
     */
    static matchBypass(pattern, context) {
        const { host } = context;
     ...
     */
    static matchHostWildcard(pattern, context) {
        const { host } = context;
     ...
     */
    static matchHostRegex(pattern, context) {
        const { host } = context;
     ...
     */
    static matchHostLevels(minValue, maxValue, context) {
        const { host } = context;
     ...
     */
    static matchUrlWildcard(pattern, context) {
        const { url } = context;
     ...
     * Uses safe regex compilation to prevent ReDoS
     */
    static matchUrlRegex(pattern, context) {
        const { url } = context;
     ...
     */
    static matchKeyword(pattern, context) {
        const { url } = context;
