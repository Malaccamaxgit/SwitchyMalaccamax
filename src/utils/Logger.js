...
class LoggerService {
    ...
    async initialize() {
        ...
        try {
            ...
            this.maxLines = Math.min(result[STORAGE_KEY_MAX_LINES] ?? DEFAULT_MAX_LINES, HARD_CAP_MAX_LINES);
                ...
                if (areaName === 'local') {
                    if (changes[STORAGE_KEY]) {
                        ...
                        if (newLevel !== undefined) {
                            ...
                            console.log(`%c[Logger] Log level changed: ${LogLevelMetadata[this.currentLevel].name}`, 'color: #8b5cf6; font-weight: bold');
                        }
                    }
                    if (changes[STORAGE_KEY_MAX_LINES]) {
                        ...
                        if (newMaxLines !== undefined) {
                            ...
                            console.log(`%c[Logger] Max lines changed: ${this.maxLines}`, 'color: #8b5cf6; font-weight: bold');
    ...
    trimPersistedLogs() {
        if (this.persistedLogs.length > this.maxLines) {
            ...
            console.log(`%c[Logger] Trimmed ${excessCount} old log entries`, 'color: #f59e0b; font-weight: bold');
        }
    }
    /**
    ...
    info(message, data) {
        ...
        if (data !== undefined) {
            console.log(`%c${this.getTimestamp()} ${formatted}`, 'color: #10b981', data);
        }
        else {
    ...
    warn(message, data) {
        ...
        if (data !== undefined) {
            console.warn(`%c${this.getTimestamp()} ${formatted}`, 'color: #f59e0b; font-weight: bold', data);
        }
        else {
    ...
    error(message, data) {
        ...
        if (data !== undefined) {
            console.error(`%c${this.getTimestamp()} ${formatted}`, 'color: #ef4444; font-weight: bold', data);
        }
        else {
    ...
    performance(label, duration) {
        ...
        console.log(`%c${this.getTimestamp()} ${formatted}: ${duration.toFixed(2)}ms`, 'color: #8b5cf6; font-style: italic');
    }
    /**
 ...
 */
export async function setLogMaxLines(maxLines) {
    // Apply hard cap
 ...
 */
export async function getLogMaxLines() {
