...
async function getEncryptionKey() {
    ...
    const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(chrome.runtime.id), 'PBKDF2', false, ['deriveKey']);
    // Generate deterministic salt from extension ID
    const salt = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(chrome.runtime.id + 'switchymalaccamax-salt'));
